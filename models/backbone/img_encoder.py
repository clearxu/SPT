...
class VPTCLIPVisionTransformer(nn.Module):
    ...
    def __init__(self, input_resolution=224, patch_size=32, width=768, layers=12, heads=12, output_dim=512, drop_path_rate=0.0, out_indices=[3, 5, 7, 11], pretrained=None, get_embeddings=False,
        ...
        # self.fre_filter = SpectralGatingNetwork(width)
        self.proj_fre = nn.Linear(1024, 10)
        nn.init.kaiming_normal_(self.proj_fre.weight, a=0, mode='fan_out')

        self.complex_weight = nn.Parameter(torch.randn(32, 17, width, 2, dtype=torch.float32) * 0.02)
    ...
    def fre_filter(self,x, cls,p=0):
        if p==1:
            x = x.permute(1,0,2)
            cls = cls.permute(1,0,2)

        x_cls = torch.mul(x, cls)
        B, N, C = x_cls.shape
        a = b = int(math.sqrt(N))
        x_cls = x_cls.view(B, a, b, C)
        x_cls = x_cls.to(torch.float32)
        x_cls = torch.fft.rfft2(x_cls, dim=(1, 2), norm='ortho')
        weight = torch.view_as_complex(self.complex_weight)
        # print(x.shape,weight.shape)
        x_cls = x_cls * weight
        x_cls = torch.fft.irfft2(x_cls, s=(a, b), dim=(1, 2), norm='ortho')

        x_cls = x_cls.reshape(B, N, C)
        if p==1:
            return x_cls.permute(1,0,2)
        return x_cls

    def forward(self, x: torch.Tensor, text_feat):
        x = self.conv1(x)
        B, C, H, W = x.shape
        x = x.reshape(x.shape[0], x.shape[1], -1)
        x = x.permute(0, 2, 1)
        ...
        if self.total_d_layer >= 0:
            prompt_fre = self.fre_filter(x[:, 1:, :], x[:, :1, :])
    ...
    def forward_deep_prompt(self, embedding_output, features, H, W,out_last=False):
        ...
        for i in range(self.num_layers):
            if i == 0:
                ...
                prompt_fre = self.fre_filter(hidden_states[(1 + self.num_tokens):, :, :],hidden_states[:1, :, :], p=1)

                if i < 2:
                    hidden_states = torch.cat((
                        hidden_states[:1, :, :],
                        deep_prompt_emb,
                        hidden_states[(1 + self.num_tokens):, :, :] + prompt_fre
                    ), dim=0)
                else:
                    hidden_states = torch.cat((
                        hidden_states[:1, :, :],
