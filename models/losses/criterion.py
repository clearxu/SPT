...
def generalized_dice_loss(pred, target):
    ...
    wei = torch.sum(target)  # (n_class,)
    wei = 1/(wei**2+epsilon)
    # intersection = torch.sum(wei*torch.sum(pred * target, axis=[0,2,3]))
    intersection = torch.sum(wei * torch.sum(pred * target))
    # union = torch.sum(wei*torch.sum(pred + target, axis=[0,2,3]))
    union = torch.sum(wei * torch.sum(pred + target))
...
class SegPlusCriterion(nn.Module):
    ...
    def loss_masks(self, outputs, targets, indices, num_masks):
        ...
        rmi_scr_masks = src_masks[:, 0].reshape(bs,n_cls,H,W)
        rmi_target_masks = target_masks.reshape(bs,n_cls,H,W)
        ...
        losses = {
            ...
            "loss_ssim": bce_ssim_loss(rmi_scr_masks,rmi_target_masks),
