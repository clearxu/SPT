...
class hlfs(nn.Module):
    ...
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., window_size=3, alpha=0.9):
        ...
        # self.ca = CrossAttentionLayer(d_model=dim, num_heads=num_heads)

        self.prompt = torch.nn.parameter.Parameter(torch.randn(dim, requires_grad=True))
        self.top_down_transform =  torch.nn.parameter.Parameter(torch.eye(dim), requires_grad=True)
    ...
    def forward(self, x, H, W, tgt):
        ...
        attn = torch.einsum('b n m, b c m -> b c n', x, tgt)
        attn_save = attn.clone()

        attn = attn.softmax(dim=-1)
        attn = self.attn_drop(attn)

        x = (attn @ x).transpose(0,1)


        # x = self.proj(x) + tgt.transpose(0, 1)
        x = self.proj(x)
